...
options.outputFile="pfNtuples.root"
options.maxEvents=-1
...
for ievent,event in enumerate(events):
    ...
    npf[0]    = pfs.product().size()

    for i,p in enumerate(pfs.product()):
        pf_pid[i] = p.pdgId()
        pf_pt[i] = p.pt()
        pf_eta[i] = p.eta()
        for idepth in range(7):
            pf_depth[i][idepth] = p.hcalDepthEnergyFraction(idepth+1)*p.energy()*p.hcalFraction()
